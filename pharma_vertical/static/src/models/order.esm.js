...
patch(Order.prototype, {
    canPay() {
        if (
            (Boolean(!this.prescriber) || Boolean(!this.prescription_date)) &&
            ...
            if (ProductScreen === this.pos.mainScreen.component) {
                ...
                this.env.services.popup.add(ConfirmPopup, {
                    title: _t("Prescription is not defined"),
                    body: _t(
                        "You need to set a prescription (prescriber and date) because some lines will be splitted"
                    ),
                });
            }
            return false;
